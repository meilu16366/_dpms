...
(function ($)
{
    ...
    $.ligerDialog.hide = function ()
    {
        ...
        {
            var d = dialogs[i];
            var val = $.ligerDialog.returnValue;
            if(val && d.options.onClosed){
            	d.options.onClosed(val);
            	$.ligerDialog.returnValue = null;
            	d.unbind("Closed");
            }
            d.hide();
            d.close();
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        checkboxColWidth: 35,                  //复选框列宽度
        detailColWidth: 35,                     //明细列宽度
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
            var g = this, p = this.options;
            if([19,20].indexOf(g.gridview.height()-$(".l-grid2").height())>-1){
            	g.gridview.height(g.gridview.height()-20);
            }
            ...
            {
            	console.info(g.isHorizontalScrollShowed());
        ...
        */
        ...
        _getColumnWidth: function (column)
        {
            ...
            if (typeof (colwidth) == "string" && colwidth.indexOf('%') > 0)
            {
                ...
                column._width = colwidth = parseInt(parseInt(colwidth) * 0.01 * (g.grid.width() - lwidth - (g.columns.length / 2) ));
            }
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (column.ischeckbox)
            {
                ...
                jcell.html("<div class='l-grid-hd-cell-inner checkbox-div'><div class='l-grid-hd-cell-text l-grid-hd-cell-btn-checkbox'></div></div>");
            }
            if (column.isdetail)
            {
        ...
        _initBuildGridHeader: function ()
        {
            ...
            g._updateFrozenWidth();
            //if(overf>100)$("div:first", g.gridheader).width(twidth);
        },
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.ischeckbox)
                    {
                        gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-checkbox" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner checkbox-div"');
                        if (p.fixedCellHeight)
                            gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
                        else
                            gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
